...
class FacialBooksApp:
    ...
    def __init__(self, db_manager):
        ...
        self._last_action    = {}          # emp_id → last-log timestamp
        self._photo_ref      = None        # keep Tk image ref alive
    ...
    def _build_dashboard_tab(self, parent):
        ...
        ctk.CTkLabel(left,
                     text="Live feed in the centre panel.\n"
        ...
        self.cam_canvas = tk.Label(
            cam_frame, bg="#0d0d0d",
            text="[ Camera not started ]", fg="#444444",
            font=("Courier New", 12))
        self.cam_canvas.grid(row=1, column=0, sticky="nsew", padx=6, pady=(0, 6))
    ...
    def _start_camera(self):
        ...
        # Request higher resolution for better face crops
        self._cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        self._cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
 
        self._camera_running = True
        self._last_action    = {}
        self.cam_status_label.configure(text="● Camera ON", text_color="#27ae60")
    ...
    def _update_camera_frame(self):
        ...
        for info in recognized:
            ...
            label = f"{name}  ({1 - info['distance']:.0%})"
            (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.60, 2)
            cv2.rectangle(rgb, (left, top - th - 10),
                          (left + tw + 6, top), color, -1)
            cv2.putText(rgb, label, (left + 3, top - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 255, 255), 2)
 
            if emp_id:
                now = time.time()
                # 60-second cooldown between automatic log attempts per employee
                if now - self._last_action.get(emp_id, 0) > 60:
        ...
        self.root.after(40, self._update_camera_frame)   # ~25 fps
    ...
    def _manual_log(self, log_type: str):
        ...
        if log_type == "entry":
            ...
            msg = (f"Exit logged for {emp_name}."
                   if ok else f"No open entry found for {emp_name}.")
    ...
    def _build_employees_tab(self, parent):
        ...
        self.reg_name = ctk.CTkEntry(form, placeholder_text="e.g. Shifa Parveen")
        self.reg_name.pack(fill="x", padx=20, pady=(0, 10))
 
        ctk.CTkLabel(form, text="Department").pack(anchor="w", padx=20)
        self.reg_dept = ctk.CTkEntry(form, placeholder_text="e.g. Engineering")
        ...
        ctk.CTkLabel(form, text="Training Photo Folder",
                     font=ctk.CTkFont(size=12, weight="bold")).pack(
            anchor="w", padx=20, pady=(10, 0))
        ctk.CTkLabel(form,
                     text="Recommended: 50–100 photos of this person.\n"
                          "Leave blank to use the webcam (50 snapshots).",
        ...
        self.reg_folder_var = ctk.StringVar(value="")
        ctk.CTkEntry(folder_row, textvariable=self.reg_folder_var,
                     placeholder_text="(leave blank for webcam)").pack(
            side="left", fill="x", expand=True)
        ctk.CTkButton(folder_row, text="Browse", width=70,
                      command=self._browse_training_folder).pack(
            side="left", padx=(6, 0))
 
        self.reg_status = ctk.CTkLabel(
            form, text="", font=ctk.CTkFont(size=12),
            text_color="#aaaaaa", wraplength=240)
        self.reg_status.pack(pady=5, padx=20)
 
        ctk.CTkButton(form, text="1. Capture / Load Faces",
                      font=ctk.CTkFont(size=12),
                      command=self._capture_faces_step).pack(
            pady=(10, 3), padx=20, fill="x")
 
        self.train_btn = ctk.CTkButton(
            form, text="2. Train CNN & Register",
            font=ctk.CTkFont(size=13, weight="bold"),
            fg_color="#1a6b3c", hover_color="#145a32",
            command=self._train_and_register)
        self.train_btn.pack(pady=(0, 10), padx=20, fill="x")
    ...
    def _browse_training_folder(self):
        folder = filedialog.askdirectory(title="Select folder with face photos")
        if folder:
            self.reg_folder_var.set(folder)
 
    def _capture_faces_step(self):
        """Step 1 — Capture or load face images."""
        name = self.reg_name.get().strip()
        if not name:
            messagebox.showwarning("Registration", "Enter employee name first.")
            return
        folder = self.reg_folder_var.get().strip()
        if folder and not os.path.isdir(folder):
            messagebox.showerror("Registration", "Folder not found. Check the path.")
            return
 
        self.reg_status.configure(text="Loading / capturing face images…")
        self.root.update()
 
        # FIX: progress_callback must be a proper function, not a nested tuple
        def _progress(count, total):
            self.reg_status.configure(text=f"Processing image {count}/{total}…")
            self.root.update()
 
        paths = self.face_engine.capture_face_images(
            name,
            num_samples=50,
            progress_callback=_progress,
            image_folder=folder if folder else None,
        )
        self._captured_paths = paths
 
        if not paths:
            self.reg_status.configure(
                text="No faces found. Check lighting / folder.")
            return
 
        self.reg_status.configure(
            text=f"✓ {len(paths)} face images ready. Click 'Train CNN & Register'.")
 
    def _train_and_register(self):
        """Step 2 — Add employee to DB, then train CNN in a background thread."""
        name     = self.reg_name.get().strip()
        dept     = self.reg_dept.get().strip()
        rate_str = self.reg_rate.get().strip()
 
        if not name or not dept or not rate_str:
            messagebox.showwarning("Registration", "Fill in all fields first.")
            return
        try:
            rate = float(rate_str)
        except ValueError:
            messagebox.showerror("Registration", "Hourly rate must be a number.")
            return
 
        paths = getattr(self, "_captured_paths", [])
        if not paths:
            messagebox.showwarning("Registration",
                                   "Run Step 1 (Capture / Load) first.")
            return
 
        # Disable button to prevent double-click
        self.train_btn.configure(state="disabled", text="Training…")
        self.reg_status.configure(text="Training CNN… this may take a few minutes.")
        self.root.update()
 
        emp_id = self.db.add_employee(name, dept, rate, [])
 
        def _progress(epoch, total, val_acc):
            # This is called from the training thread → schedule on main thread
            self.root.after(
                0,
                lambda e=epoch, t=total, v=val_acc: (
                    self.reg_status.configure(
                        text=f"Epoch {e}/{t}  |  val_acc = {v:.1%}"),
                    self.root.update(),
                ),
            )
 
        def _train_thread():
            ok = self.face_engine.train_model(
                employee_id=emp_id,
                employee_name=name,
                image_paths=paths,
                progress_callback=_progress,
            ...
            self.root.after(0, lambda: self._on_training_done(ok, emp_id, name))
 
        threading.Thread(target=_train_thread, daemon=True).start()
 
    def _on_training_done(self, success: bool, emp_id: int, name: str):
        """Called on the main thread after training finishes."""
        self.train_btn.configure(state="normal", text="2. Train CNN & Register")
 
        if not success:
            self.db.delete_employee(emp_id)
            self.reg_status.configure(text="Training failed. Need ≥ 30 face images.")
            messagebox.showerror(
                "Training Failed",
                "Could not train the CNN.\n\n"
                "Make sure:\n"
                "  • At least 30 clear face photos\n"
                "  • Add other-people photos to ./negatives/\n"
                "  • Good lighting, front-facing shots")
            return
 
        self.face_engine.refresh_encodings()
        self.reg_status.configure(
            text=f"✓ {name} registered & CNN trained!  (ID: {emp_id})")
 
        self.reg_name.delete(0, "end")
        self.reg_dept.delete(0, "end")
        self.reg_rate.delete(0, "end")
        self.reg_folder_var.set("")
    ...
    def _build_attendance_tab(self, parent):
        ...
        ctk.CTkLabel(ctrl, text="Month (YYYY-MM):").pack(side="left", padx=10)
        ...
        ctk.CTkButton(ctrl, text="Search",
                      command=self._load_attendance).pack(side="left", padx=10)
    ...
    def _populate_att_filter(self):
        employees = self.db.get_all_employees()
        options   = ["All"] + [f"{e['name']} [{e['id']}]" for e in employees]
    ...
    def _build_payroll_tab(self, parent):
        ...
        ctk.CTkButton(ctrl, text="Generate Payroll",
                      fg_color="#1a6b3c", hover_color="#145a32",
                      command=self._generate_payroll).pack(side="left", padx=10)
        ctk.CTkButton(ctrl, text="Export Excel",
